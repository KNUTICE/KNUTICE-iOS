...
extension MainTableViewController {
    ...
    func navigateToAcademicNotice(_ sender: UIButton) {
        let viewController = NoticeCollectionViewController(
            viewModel: NoticeCollectionViewModel(category: NoticeCategory.academicNotice),
            navigationTitle: "학사공지"
        )
        navigationController?.pushViewController(viewController, animated: true)
    }
    ...
    func navigateToScholarshipNotice(_ sender: UIButton) {
        let viewController = NoticeCollectionViewController(
            viewModel: NoticeCollectionViewModel(category: NoticeCategory.scholarshipNotice),
            navigationTitle: "장학안내"
        )
        navigationController?.pushViewController(viewController, animated: true)
    }
    ...
    func navigateToEventNotice(_ sender: UIButton) {
        let viewController = NoticeCollectionViewController(
            viewModel: NoticeCollectionViewModel(category: NoticeCategory.eventNotice),
            navigationTitle: "장학안내"
        )
        navigationController?.pushViewController(viewController, animated: true)
    }
    ...
    func navigateToEmploymentNotice() {
        let viewController = NoticeCollectionViewController(
            viewModel: NoticeCollectionViewModel(category: NoticeCategory.employmentNotice),
